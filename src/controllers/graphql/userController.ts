...
class UserController {
  ...
  async register(parent: any, { input }: { input: IUserInput }, contextValue: any, info: any) {
    try {
  ...
  async enable_2fa(parent: any, { input }: { input: IUserInput }, contextValue: any, info: any) {
    try {
      const { email, password } = input;
  ...
  async login_2fa(parent: any, { input }: { input: ICodeInput }, contextValue: any, info: any) {
    try {
  ...
  async update(parent: any, { input }: { input: IUpdateInput }, contextValue: any, info: any) {
    try {
  ...
  // logout resolver
  async logout(parent: any, input: any, contextValue: any, info: any) {
    try {
  ...
  async refresh(parent: any, input: any, contextValue: any, info: any) {
    try {
  ...
  // get protected resource (only authorized users can do it)
  async get_protected_resource(parent: any, input: any, contextValue: any, info: any) { 
    try {
      const user_id = contextValue.req.user?.userId;
      ...
      return { message: `User with id ${user_id} got protected resource` };
    } catch (error) {
      if (error instanceof Error) {
        // Handle the error
        throw APIError.internal(error.message);
